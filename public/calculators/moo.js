...
const minsSincePreorder = Math.floor(
  (Date.now() - preorder_date.getTime()) / 1000 / 60,
);
...
async function model(data, trends_data, parameters, model_totals = false) {
  ...
  const modelDemandScalar = model_totals
    ? 1
    : modelDemandRatio[parameters.model];
  const regionDemandScalar = model_totals
    ? 1
    : regionDemandRatio[parameters.region];

  //Filter by order queue

  const newData = model_totals
    ? data
    : data.whereModelIs(parameters.model).whereRegionIs(parameters.region);
  ...
  for (let i = 0; i < minsSincePreorder; i++) {
    ...
    for (const item of data.responses) {
      if (
        item.rtReserveTime >= last_timestamp && item.rtReserveTime < timestamp
      ) {
        order_quantity[i] = order_quantity[i] +
          (averageScalar * modelDemandScalar * regionDemandScalar);
      }
    }

    //Add in trends data.. This is truely magic numbers.
    order_quantity[i] = order_quantity[i] +
      (trends_data[i] * modelDemandScalar * regionDemandScalar);

    //Do some data smoothing to get more realistic numbers.
    //Moving average.
    if (order_quantity[i] < last_quantity) {
      order_quantity[i] = (order_quantity[i] + last_quantity + last_quantity) /
        3;
    }
  ...
  if (parameters.model == "64") {
    deliveriesQ1 = getOrdersThatAreInTimeFrame(data, "Q1") * us64 *
      modelDemandScalar * regionDemandScalar;
    deliveriesQ2 = getOrdersThatAreInTimeFrame(data, "Q2") * us64 *
      modelDemandScalar * regionDemandScalar;
  }
...
export async function mooV2(parameters, data) {
  ...
  const user_order_position = ordersBetweenMins(
    model_result.order_quantities,
    0,
    user_order_mins,
  );
...
async function getGoogleTrendsData() {
  const data = await (await fetch("2-12-22-google-trends-steam-deck.csv"))
    .text();
  ...
  for (let x = 0; x < mins.length; x++) {
    ...
    for (let i = 0; i < processedData.length - 1; i++) {
      ...
      if (
        minTime.getTime() >= currentDuration && minTime.getTime() < nextDuration
      ) {
        //Do the math
        ...
        const weightedAverage = (((1 - score) * value) + (nextValue * score)) /
          2;
...
async function test_moo_model() {
  ...
  for (const test of tests) {
    ...
    const delta = (test.expected_result.getTime() - result.getTime()) /
      (1000 * 86400);
    ...
    console.log(
      `${test.name}: expected ${test.expected_result.toDateString()} but got ${result.toDateString()} delta ${delta}`,
    );
  }
